...
static string opcode_to_hexadecimal(opcode code)
{
	return "0x" + uint8_t_to_string(static_cast<uint8_t>(code));
}
...
string opcode_to_string(opcode value, uint32_t active_forks)
{
	...
	case opcode::push_size_75:
		return "push_" + to_string(static_cast<uint8_t>(value));
	...
	case opcode::push_positive_16:
		return to_string(static_cast<uint8_t>(value) - push_zero);
	case opcode::nop:
		return "nop";
...
string decode_script_(const string& str_script)
{
	string str_op, str_out;
	size_t index = 0;
	for (index = 0; index < str_script.size();)
	{
		str_op = str_script.substr(index, 2);
		index += 2;
		uint8_t u8_op = string_to_uint8_t(str_op);
		if (u8_op > static_cast<uint8_t>(opcode::push_positive_16))
		{
			str_out += opcode_to_string(static_cast<opcode>(u8_op), rule_fork::all_rules);
			if (index != str_script.size())
				str_out += " ";
		}
		else
		{
			string str_pubkey_hash = str_script.substr(index, u8_op * 2);
			index += u8_op * 2;
			str_out = str_out + "[" + str_pubkey_hash + "] ";
		}
	}
...
bool opcode_from_hexadecimal(opcode& out_code, const string& value)
{
	if (value.size() != 4 || value[0] != '0' || value[1] != 'x')
		return false;

	// data_chunk out;
	Binary out= Binary::decode(string(value.begin() + 2, value.end()));
	out_code = static_cast<opcode>(out.front());
	return true;
}

bool opcode_from_string(opcode& out_code, const string& value)
{
	string norm;
	transform(value.begin(), value.end(), back_inserter(norm), ::tolower);
...
string encode_script_(const string& str_script)
{
	stringstream ss(str_script);
	string str_op, str_res;
	...
	while (true)
	{
		...
		opcode opcode_op;
		bool ok = opcode_from_string(opcode_op, str_op);
		if (ok)
		{
			uint8_t u8_op = static_cast<uint8_t>(opcode_op);
			if (u8_op > static_cast<uint8_t>(opcode::push_positive_16))
				str_res += uint8_t_to_string(u8_op);
		}
		else
		{
			string str_pubkey_hash = str_op.substr(1, str_op.size() - 2);
			str_res = str_res + uint8_t_to_string((uint8_t)(str_pubkey_hash.size() / 2)) + str_pubkey_hash;
		}
	}
