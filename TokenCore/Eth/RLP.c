...
int wallet_copy_rpl(uint8_t *source, uint8_t *destination, uint8_t size, int copyPos)
{
    int ret_val = copyPos;
    if (size != 0) {
...
int wallet_encode_list(EncodeEthereumSignTx *new_msg, uint64_t *rawTx)
{
    ...
    totalLength += new_msg->data.size;

    totalLength += new_msg->signature_v.size;
    totalLength += new_msg->signature_r.size;
    totalLength += new_msg->signature_s.size;
    ...
    copyPos = wallet_copy_rpl(data + copyPos, new_msg->data.bytes,
                              new_msg->data.size, copyPos);
    copyPos = wallet_copy_rpl(data + copyPos, new_msg->signature_v.bytes, new_msg->signature_v.size, copyPos);
    copyPos = wallet_copy_rpl(data + copyPos, new_msg->signature_r.bytes,
                              new_msg->signature_r.size, copyPos);
    copyPos = wallet_copy_rpl(data + copyPos, new_msg->signature_s.bytes,
                              new_msg->signature_s.size, copyPos);
